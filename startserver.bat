...
IF NOT EXIST "%cd%\serverstarter-2.1.1.jar" (
	...
	%SYSTEMROOT%\SYSTEM32\bitsadmin.exe /rawreturn /nowrap /transfer starter /dynamic /download /priority foreground https://github.com/P1XZL/Serverstarter/raw/main/serverstarter-2.1.1.jar "%cd%\serverstarter-2.1.1.jar"
   GOTO MAIN
) ELSE (
   GOTO MAIN
)
